...
const CLASS_ID = Components.ID("0af96550-7bf7-11e0-819a-0800200c9a66");
  ...
  startSearch: function(searchString, searchParam, result, listener) {
    ...
    if (searchParam.length > 0) {
      ...
      for (i=0; i<searchResults.length; i++) {
		if (searchResults[i].value.toUpperCase().indexOf(searchString.toUpperCase()) == 0) {
			...
			if (searchResults[i].image) {
				images.push(searchResults[i].image);
			} else {
				images.push(null);
			}
		}
	  }
      var newResult = new SimpleAutoCompleteResult(searchString, Ci.nsIAutoCompleteResult.RESULT_SUCCESS, 0, "", results, comments, images);
